                    ...
                    <div className="flex items-center gap-3">
                        {user.avatarUrl ? (
                            <img
                                src={user.avatarUrl}
                                alt={user.fullname}
                                className="w-10 h-10 rounded-full object-cover shadow-sm border-2 border-white dark:border-gray-700"
                            />
                        ) : (
                            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-lg ${theme === 'dark' ? 'bg-gray-600 text-gray-200' : 'bg-slate-600 text-white'
                                }`}>
                ...
                <div className="p-6">
                    <div className="grid grid-cols-2 gap-4 mb-6">
                        <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            <label className={`flex items-center gap-2 text-xs font-semibold mb-2 uppercase tracking-wider ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                                <UserIcon size={14} />
                                Tài khoản
                            </label>
                            <p className={`text-sm font-mono px-2.5 py-1.5 rounded border ${theme === 'dark' ? 'bg-gray-800 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-700'}`}>
                                {user.username}
                            </p>
                        </div>
                        ...
                        <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <p className={`text-sm px-2.5 py-1.5 rounded border break-all ${theme === 'dark' ? 'bg-gray-800 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-700'}`}>
                                {user.email}
                            </p>
                        </div>
                        ...
                        <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <p className={`text-sm px-2.5 py-1.5 rounded border ${theme === 'dark' ? 'bg-gray-800 border-gray-600 text-gray-200' : 'bg-white border-gray-300 text-gray-700'}`}>
                                {user.phoneNumber}
                            </p>
                        </div>
                        ...
                        <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            ...
                            <div className={`text-sm font-semibold px-2.5 py-1.5 rounded border text-center ${getRoleBadgeColor(user.role)}`}>
                                {getRoleLabel(user.role)}
                            </div>
                        </div>

                        <div className={`p-4 rounded-lg border ${theme === 'dark' ? 'bg-gray-700 border-gray-600' : 'bg-gray-50 border-gray-200'}`}>
                            <label className={`block text-xs font-semibold mb-2 uppercase tracking-wider ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'}`}>
                                Trạng thái
                            </label>
